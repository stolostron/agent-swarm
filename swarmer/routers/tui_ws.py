...
async def session_tui(
    ...
    # Inject or remove mcp-auth.json before starting the TUI binary.
    # If the session has Jira MCP disabled we actively remove any stale copy
    # that may have been left on a persistent PVC from a previous launch.
    _MCP_AUTH_PATH = "/workspace/.local/share/opencode/mcp-auth.json"
    mcp_auth_preamble = ""
    if not session.enable_jira_mcp:
        # Actively remove stale file so OpenCode doesn't pick up a cached token.
        mcp_auth_preamble = f"rm -f {_MCP_AUTH_PATH} && "
    elif hasattr(tool, "build_mcp_auth_setup_cmd"):
        try:
