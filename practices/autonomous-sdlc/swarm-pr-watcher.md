...
| Category | Author Scope | Condition / Trigger | Prompt |
| :--- | :--- | :--- | :--- |
| **My PRs** | `self` (per-schedule `fix_authors` GitHub logins) | Any configured event condition | Selected schedule Prompt |
| **Team PRs** | `team` (trusted collaborators) | Any configured event condition | Selected schedule Prompt |
| **Automated Bots** | `bots` (`dependabot`, `renovate`, `app/*`) | Any configured event condition | Selected schedule Prompt |
| **All PRs** | `all` (any author) | Any configured event condition | Selected schedule Prompt |

Conditions and author scopes are independent. The watcher evaluates the event
condition first, then the author scope. The selected Prompt determines the
agent behavior. A fork PR remains eligible, and the event context reports
`fork_no_push` when the base repository cannot push to it.
...
- **CI Completion Barrier:** Checks must have 0 `in_progress` or `queued` check runs plus a 90-second quiet debounce period before a matching schedule is dispatched.
- **Circuit Breaker:** Maximum 3 attempts per event condition and `head_sha`. If the agent fails after 3 attempts, the status is marked `blocked`. A new human commit to the branch resets the counter.
...
- **Fork Visibility:** Fork PRs are still dispatched when their configured condition and author scope match. The event context exposes `fork_no_push` so the selected Prompt can account for limited write access.
