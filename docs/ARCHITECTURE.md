...
3. **Kubeconfig Context** (`$KUBECONFIG` or default user kubeconfig path):
...
- **CLI Automation (`make mcp-setup` / `make api-info`):**
  - `make mcp-setup`: Interactively prompts for or receives piped bearer credentials and configures `opencode.json` with the detected Swarmer route without persisting plaintext tokens.
  - `make api-info`: Prints current API endpoint, decoded user identity, and the MCP JSON snippet with redacted tokens.
